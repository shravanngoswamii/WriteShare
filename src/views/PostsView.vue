...
const repoUrl = `https://github.com/${CMS_CONFIG.repo.owner}/${CMS_CONFIG.repo.repo}`;
...
onMounted(async () => {
  ...
  await refreshPosts(collection.path);
});
...
const entries = computed(() => {
  ...
  return postsState.files
    .map((path) => {
      const rel = path.slice(collection.path.length + 1);
...
<template>
  <div class="page">
    <div class="topbar">
      ...
      <a class="chip" :href="repoUrl" target="_blank" rel="noreferrer">{{ CMS_CONFIG.repo.repo }}</a>
      <img
        v-if="auth.user"
        class="avatar"
    ...
    <div v-if="composing" class="composer">
      ...
      <input v-model="newPost.folder" type="text" placeholder="Folder (optional)" list="folders" @keydown.enter="createNew" />
      <datalist id="folders">
    ...
    <p v-else class="muted list-note small">{{ entries.length }} posts</p>
...
/* Apple-style grouped list: paper container, hairline separators, no borders */
.grouped {
  background: var(--paper);
  border-radius: var(--radius-lg);
  box-shadow: var(--shadow-card);
